...
class CacheOpenAI(BaseLLM):
    ...
    @classmethod
    def from_experiment_config(cls, global_config: BaseConfig, llm_name: str = None, 
                               llm_base_url: str = None) -> "CacheOpenAI":
        config_dict = global_config.__dict__
        config_dict['max_retries'] = global_config.max_retry_attempts
        cache_dir = os.path.join(global_config.save_dir, "llm_cache")
        return cls(cache_dir=cache_dir, global_config=global_config, llm_name=llm_name, 
                   llm_base_url=llm_base_url)

    def __init__(self, cache_dir, global_config, cache_filename: str = None,
                 high_throughput: bool = True, llm_name: str = None,
        ...
        self.llm_name = llm_name if llm_name is not None else global_config.llm_name
        self.llm_base_url = llm_base_url if llm_base_url is not None else global_config.llm_base_url
        ...
        if high_throughput:
            ...
            client = httpx.Client(limits=limits, timeout=httpx.Timeout(None,connect=10.0, read=1200.0))
        else:
            client = None
    ...
    def _cleanup_response_message(self, text: str) -> str:
        """
        一个健壮的清理函数，专门对付具有“思考-回答”模式的模型。
        它会精准地提取 [unused17] 之后的内容。
        """
        # 定义“思考结束，答案开始”的标记
        answer_start_token = '[unused17]'
        
        # 查找这个标记在字符串中的位置
        start_index = text.find(answer_start_token)
        
        # 如果找到了这个标记
        if start_index != -1:
            cleaned_text = text[start_index + len(answer_start_token):]
        else:
            return ""
    ...
    def infer(
        ...
        **kwargs
        ...
        if "pangu" in params['model']:
            response_message = self._cleanup_response_message(response_message)
