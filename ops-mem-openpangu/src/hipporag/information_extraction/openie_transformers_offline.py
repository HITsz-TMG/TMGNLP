...
class TransformersOfflineOpenIE(OpenIE):
    ...
    def batch_openie(self, chunks: Dict[str, ChunkInfo], triple_prompt_name: str = 'triple_extraction') -> Tuple[Dict[str, NerRawOutput], Dict[str, TripleRawOutput]]:
        ...
        json_template_name = 'triples'
        if triple_prompt_name == 'triple_extraction_with_time':
            json_template_name = 'triples_with_time'
            
        triple_output, triple_output_metadata = self.llm_model.batch_infer(triple_extract_input_messages, json_template=json_template_name, max_tokens=2048)
