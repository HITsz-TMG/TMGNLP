...
class OpenIE:
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str], triple_prompt_name: str = 'triple_extraction') -> TripleRawOutput:
        def _extract_triples_from_response(real_response):
            ...
            if match is None:
                # If pattern doesn't match, return an empty list
                return []
            # Use json.loads instead of eval to properly handle null values
            try:
                return json.loads(match.group())["triples"]
            except json.JSONDecodeError:
                # Fallback to eval if json.loads fails (for backward compatibility)
                # Replace null with None for Python compatibility
                json_str = match.group().replace('null', 'None')
                return eval(json_str)["triples"]

        # PREPROCESSING
        messages = self.prompt_template_manager.render(
            name=triple_prompt_name,
            passage=passage,
    ...
    def batch_openie(self, chunks: Dict[str, ChunkInfo], triple_prompt_name: str = 'triple_extraction') -> Tuple[Dict[str, NerRawOutput], Dict[str, TripleRawOutput]]:
        """
        ...
        Args:
            ...
            triple_prompt_name (str): The name of the prompt template to use for triple extraction.
        ...
        with ThreadPoolExecutor() as executor:
            ...
            re_futures = {
                executor.submit(self.triple_extraction, ner_result.chunk_id,
                                ...
                                triple_prompt_name): ner_result.chunk_id
