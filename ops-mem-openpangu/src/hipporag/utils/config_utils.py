...
class BaseConfig:
    ...
    rerank_llm_base_url: str = field(
        default=None,
        metadata={"help": "Base URL for the rerank LLM model, separate from the main LLM."}
    )
    rerank_model_name: str = field(
        default="Qwen/Qwen3-Reranker-4B",
        metadata={"help": "Name of the rerank model to use."}
    )
    ...
    generate_episodic_memory: bool = field(
        default=False,
        metadata={"help": "Whether to generate episodic memory for chunks during indexing."}
    )
    use_episodic_memory: bool = field(
        default=True,
        metadata={"help": "Whether to use episodic memory during retrieval and QA. If False, episodic memories will not be loaded or used even if they exist."}
    )
    episodic_memory_llm_name: Optional[str] = field(
        default=None,
        metadata={"help": "LLM model name for episodic memory generation. If None, uses the main LLM."}
    )
    episodic_memory_batch_size: int = field(
        default=10,
        metadata={"help": "Batch size for episodic memory generation."}
    )
    episodic_memory_cache_enabled: bool = field(
        default=True,
        metadata={"help": "Whether to enable caching for episodic memory generation."}
    )
    episodic_memory_embedding_batch_size: int = field(
        default=32,
        metadata={"help": "Batch size for computing episodic memory embeddings."}
    )
    related_chunks_top_k: int = field(
        default=5,
        metadata={"help": "Number of candidate chunks to pre-select using KNN for related chunk identification."}
    )
    related_chunks_strategy: Literal['embedding_llm', 'entity', 'episodic', 'hybrid'] = field(
        default='embedding_llm',
        metadata={"help": "Strategy for finding related chunks: 'embedding_llm' (embedding similarity + LLM filter), 'entity' (entity-based), 'episodic' (episodic element-based), 'hybrid' (mixed strategy)."}
    )
    related_chunks_llm_filter: bool = field(
        default=True,
        metadata={"help": "Whether to use LLM filtering after KNN pre-selection for related chunk identification. If False, only KNN is used."}
    )
    # 多对一情境记忆相关配置
    enable_multi_chunk_memory: bool = field(
        default=True,
        metadata={"help": "Whether to enable multi-to-one episodic memory mode (N chunks -> 1 memory)."}
    )
    memory_integration_similarity_threshold: float = field(
        default=0.7,
        metadata={"help": "Similarity threshold for memory integration (>= this threshold will be considered for integration)."}
    )
    memory_integration_llm_filter: bool = field(
        default=True,
        metadata={"help": "Whether to use LLM filtering for integration judgment."}
    )
    # 关联关系相关配置
    memory_relation_similarity_min: float = field(
        default=0.6,
        metadata={"help": "Minimum similarity threshold for memory relationships."}
    )
    memory_relation_similarity_max: float = field(
        default=0.7,
        metadata={"help": "Maximum similarity threshold for memory relationships (should be lower than integration threshold)."}
    )
    memory_relation_max_per_memory: int = field(
        default=5,
        metadata={"help": "Maximum number of related memories per memory."}
    )
    memory_relation_llm_filter: bool = field(
        default=True,
        metadata={"help": "Whether to use LLM filtering for relationship judgment."}
    )
    # Episodic element extraction configuration
    extract_when: bool = field(
        default=True,
        metadata={"help": "Whether to extract time information (when) in episodic elements."}
    )
    extract_where: bool = field(
        default=True,
        metadata={"help": "Whether to extract location information (where) in episodic elements."}
    )
    extract_why: bool = field(
        default=True,
        metadata={"help": "Whether to extract reason/motivation (why) in episodic elements."}
    )
    extract_how: bool = field(
        default=True,
        metadata={"help": "Whether to extract method/manner (how) in episodic elements."}
    )
    max_who_entities: int = field(
        default=10,
        metadata={"help": "Maximum number of entities to extract in the 'who' field of episodic elements."}
    )
    max_what_concepts: int = field(
        default=10,
        metadata={"help": "Maximum number of concepts to extract in the 'what' field of episodic elements."}
    )
    ...
    qa_backfill_max_additional_chunks: int = field(
        default=20,
        metadata={"help": "In QA episodic-memory mode, max number of additional chunks to backfill into Original Passages based on episodic memories."}
    )
