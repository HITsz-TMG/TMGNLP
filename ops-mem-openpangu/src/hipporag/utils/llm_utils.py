...
def filter_invalid_triples(triples: List[List[str]]) -> List[List[str]]:
    ...
    for triple in triples:
        if len(triple) not in (3, 4): continue  # Skip triples that do not have exactly 3 or 4 elements

        valid_triple = [str(item) if item is not None else "" for item in triple] # Handle None for time
...
class TriplesWithTimeModel(pydantic.BaseModel):
    """
    For triples extraction with time, the structure is:
    schema:
    {
        "type": "object",
        "properties": { "triples": { "type": "array", "items": { "type": "array", "items": [str, str, str, str|null], "maxItems": 4, "minItems": 4 } } },
        "required": ["triples"]
    }
    """
...
MODEL_TEMPLATES: Dict[str, Type[pydantic.BaseModel]] = {
    ...
    "triples_with_time": TriplesWithTimeModel,
    "fact": FactModel,
