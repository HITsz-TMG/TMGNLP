...
def run_dataset(dataset_name, log_dir, run_id, base_save_dir, cmd_base_args):
    ...
    # 构建唯一的save_dir路径
    unique_save_dir = os.path.join(base_save_dir, dataset_name)
    ...
    # 获取数据集 QA 数量
    qa_count = 0
    dataset_file = f"longmemeval_s/{dataset_name}.json"
    try:
        import json
...
def main():
    ...
    parser.add_argument('--base_save_dir', type=str, default='outputs_longmemeval_s',
                        help='Base directory for saving outputs (default: outputs_longmemeval_s)')
    ...
    batch_log_dir = os.path.join(base_log_dir, f"batch_longMemEval_{run_id}")
    ...
    datasets = [f"longmemeval_s_{i}" for i in range(500)]
