...
def flashattn(batch, heads, heads_kv, dim, dim_v):
    ...
    def kernel_func(block_N, block_H, num_split_sparse, num_stages, threads, max_cache_seqlen,
                    max_selected_blocks, num_total_blocks, full_head_num):

        num_split_full = int(math.ceil(num_split_sparse * num_total_blocks / max_selected_blocks))

        total_splits = full_head_num * num_split_full + (heads_kv - full_head_num) * num_split_sparse
        num_split = num_split_sparse if full_head_num == 0 else num_split_full

        shape_q = [batch, heads, dim]
        shape_k = [batch, max_cache_seqlen, heads_kv, dim]
        shape_v = [batch, max_cache_seqlen, heads_kv, dim_v]
        shape_indices = [batch, heads_kv, num_total_blocks]
        ...
        def flash_attn_split(
            ...
            with T.Kernel(batch, total_splits, threads=threads) as (bx, b_combined):
                ...
                is_full = b_combined < full_head_num * num_split_full
                hid = T.if_then_else(is_full, b_combined // num_split_full, (b_combined - full_head_num * num_split_full) // num_split_sparse + full_head_num)
                sid = T.if_then_else(is_full, b_combined - hid * num_split_full, b_combined - full_head_num * num_split_full - (hid - full_head_num) * num_split_sparse)
                ...
                num_blocks = T.if_then_else(is_full, num_total_blocks, max_selected_blocks)
                num_split = T.if_then_else(is_full, num_split_full, num_split_sparse)
        ...
        def combine(
                ...
                # is_full_head_map: T.Tensor([heads_kv], "bool")
        ):
            with T.Kernel(heads, batch, threads=128) as (by, bz):
                cur_kv_head = by // kv_group_num
                is_full = cur_kv_head < full_head_num
                num_split = T.if_then_else(is_full, num_split_full, num_split_sparse)
...
def get_configs():
    """Define the configuration space for block_H, num_stages, and threads."""
    block_H_candidates = [64, 128]
    num_stages_candidates = [1, 2]
    threads_candidates = [128, 256]
    num_split_sparse_candidates = [1, 2, 4, 8, 16]
    configs = [
        {"block_H": block_H, "num_stages": num_stages, "threads": threads, "num_split_sparse": num_split_sparse}
        for block_H in block_H_candidates
        for num_stages in num_stages_candidates
        for threads in threads_candidates
        for num_split_sparse in num_split_sparse_candidates
    ]
    return configs
...
def get_best_config(batch, heads, heads_kv, dim, dim_v, block_size, max_cache_seqlen, sparse_ratio, full_head_num):
    """Use AutoTuner to find the best configuration based on performance."""
    def kernel_wrapper(block_H, num_stages, threads, num_split_sparse):

        max_selected_blocks = int(math.ceil(max_cache_seqlen * (1 - sparse_ratio) / block_size))
        num_total_blocks = int(math.ceil(max_cache_seqlen / block_size))
        program = flashattn(batch, heads, heads_kv, dim, dim_v)(
            block_N=block_size,
            block_H=block_H,
            num_split_sparse=num_split_sparse,
...
class SparseFlashAttn(torch.nn.Module):
    ...
    def __init__(self, batch, heads, heads_kv, dim, dim_v, block_size, max_cache_seqlen, sparse_ratio, full_head_num):
        ...
        self.full_head_num = full_head_num

        # Get the best configuration using autotune
        best_config = get_best_config(batch, heads, heads_kv, dim, dim_v, block_size, max_cache_seqlen, sparse_ratio, full_head_num)
        self.kernel = best_config.kernel



        props = torch.cuda.get_device_properties(torch.device("cuda:0"))
        self.num_sm = props.multi_processor_count
        self.block_H = best_config.config['block_H']
        self.num_split_sparse = best_config.config['num_split_sparse']
    ...
    def forward(self, query, key, value, block_indices, cache_seqlens):
        ...
        max_cache_seqlen = self.max_cache_seqlen
        num_split_sparse = self.num_split_sparse
        max_selected_blocks = int(math.ceil(max_cache_seqlen * (1 - sparse_ratio) / block_size))
        num_total_blocks = int(math.ceil(max_cache_seqlen / block_size))
        num_split_full = int(math.ceil(num_split_sparse * num_total_blocks / max_selected_blocks))
        num_split = num_split_sparse if self.full_head_num == 0 else num_split_full
...
def ref_program_torch_hybrid_head(query, key, value, block_indices, cache_seqlens, max_cache_seqlen, num_blocks,
                                  block_size, full_head_num):
    heads = query.shape[1]
    heads_kv = key.shape[2]
    kv_group_num = heads // heads_kv
    if full_head_num > 0:
        out_full = ref_program_fa(query[:,:full_head_num*kv_group_num,], key[:,:,:full_head_num,], value[:,:,:full_head_num,], block_indices[:,:full_head_num,], cache_seqlens, max_cache_seqlen, num_blocks, block_size)
    else:
        out_full = None
    if heads_kv - full_head_num > 0:
        out_sparse = ref_program_torch(query[:, full_head_num * kv_group_num:, ], key[:, :, full_head_num:, ],value[:, :, full_head_num:, ], block_indices[:, full_head_num:, ], cache_seqlens, max_cache_seqlen, num_blocks, block_size)
    else:
        out_sparse = None
    ...
    if out_full is None:
        ...
        out = torch.cat([out_full, out_sparse], dim=1)
    return out
...
def main(batch=8,
    ...
    max_selected_block = int(math.ceil(max_cache_seqlen * (1 - sparse_ratio) / block_size))
    num_total_blocks = int(math.ceil(max_cache_seqlen / block_size))
    dtype = torch.float16
    ...
    block_indices = torch.full((batch, heads_kv, num_total_blocks),
                               -1,
                               dtype=torch.int32,
                               device='cuda')
    ...
    for b in range(batch):
        ...
        if max_valid_block > 0:  # Ensure there's at least one valid block
            for h in range(heads_kv):
                if h < full_head_num:
                    # For full attention heads, select all valid blocks
                    valid_indices = torch.arange(max_valid_block, device='cuda', dtype=torch.int32)
                else:
                    # For sparse heads, select a random subset of blocks
                    valid_indices = torch.randperm(
                        max_valid_block, device='cuda', dtype=torch.int32)[:max_selected_block]
    ...
    # parity reference
    ref = ref_program_torch_hybrid_head(Q, K, V, block_indices, cache_seqlens, max_cache_seqlen, max_num_blocks,
                                        block_size, full_head_num)

    sparse_kernel = SparseFlashAttn(batch, heads, heads_kv, dim, dim_v, block_size, max_cache_seqlen, sparse_ratio, full_head_num)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--max_cache_seqlen', type=int, default=64000, help='kvcache sequence length')
    ...
    parser.add_argument('--full_head_num', type=int, default=1, help='full_head_num')
    torch.manual_seed(42)
    args = parser.parse_args()
    main(args.batch, args.heads, args.heads_kv, args.max_cache_seqlen, args.dim, args.dim_v,
         args.sparse_ratio, args.block_size, args.full_head_num)