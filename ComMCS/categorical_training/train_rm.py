...
from trainer_utils import MCSoftTrainer, LlamaForMCS
...
class MCSoftDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(data_path, list):
            ...
            self.raw_data = self.preprocess_function(raw_data)
        else:
    ...
    def preprocessing(self, example):
        ...
        for step, score in zip(example['steps'], example['score']):
            new_input_tensor = self.tokenizer.encode(step + example['splitter'])[1:]
            new_labels = [self.ignore_index] * (len(new_input_tensor)-1) + [score]
...
if __name__ == "__main__":
    ...
    train_dataset = MCSoftDataset(data_path=train_path, tokenizer=tokenizer, sample_size=-1, model_max_length=training_args.max_seq_length)
    eval_dataset = MCSoftDataset(data_path=test_path, tokenizer=tokenizer, sample_size=-1, model_max_length=training_args.max_seq_length)
